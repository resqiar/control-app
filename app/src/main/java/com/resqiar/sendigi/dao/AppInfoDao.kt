...
interface AppInfoDao {
    ...
    @Query(
        "UPDATE app_info SET lock_status= :lockStatus, lock_dates = :lockDates, lock_start_time = :lockStartTime, lock_end_time = :lockEndTime, recurring = :recurring WHERE package_name = :packageName"
    )
    suspend fun updateScheduler(packageName: String, lockDates: String, lockStartTime: String, lockEndTime: String, lockStatus: Boolean, recurring: String)

    @Query(
        "UPDATE app_info SET lock_status = :lockStatus, lock_dates = :dateLocked, lock_start_time = :timeStartLocked, lock_end_time = :timeEndLocked, recurring = :recurring WHERE package_name = :packageName"
    )
